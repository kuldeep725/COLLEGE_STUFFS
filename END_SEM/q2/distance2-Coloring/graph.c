...
void setDegrees (GRAPH *graph, int arr[][32], int degree[]) {
...
//program cut-vertices functions begin here

void findCutVertices (GRAPH *graph) {

    int path[32][32] = {0};
    dfsUsingRecursion (graph, 0, path);
    ...
    int visited[32] = {0};
    int degree[32] = {0};
    int cut_vertices[32] = {0};

    setDegrees (graph, path, degree);
    ...
    //         if (path[i][j] == -5 && graph->a[i][j] != 0 && !visited[j]) {
...
int isColorSameNew (GRAPH *graph, int vertexColor[], int v) {
    ...
    for (i = 0; i < graph->n; ++i) {
        ...
        if ((BFS(graph, v, i) == 2 || BFS(graph, v, i) == 1 || graph->a[i][v] == 1 || graph->a[v][i] == 1) && vertexColor[v] == vertexColor[i]) return i;             //color of vertex adjacent
                                                                    //to v has same color
    }
    return -1;               //no adjacent vertex of v has the same color as v
