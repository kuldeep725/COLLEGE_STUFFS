...
void printPathForDijkstra (GRAPH *graph, int parent[], int dest, int a_d[][32]) {
    ...
    // printf("dist[%d] = %d\n",dest, dist[dest]);
    printf("%d, %d\n", parent[dest], dest);
    a_d[parent[dest]][dest] = - 5;
    a_d[dest][parent[dest]] = - 5;
        ...
        for (j = i; j < graph->n; j++) {
            ...
            if (graph->a[i][j] != 0) {
                ...
                if (a_d[i][j] == -5 || a_d[j][i] == -5) {
                    printf("if (%d, %d)\n", i, j);
                    fprintf(fpWrite, "[color=red, label = %d] ", graph->a[i][j]);
                ...
                /*if (graph->a[i][j] == -5) {
                    printf("(%d, %d)\n", i,j);
                    printf("marked[%d] = %d, marked[%d] = %d\n", i, marked[i], j, marked[j]);
                    ...
                    if (marked[i] == 0 && marked[j] == 0) {
                        ...
                        if (i == root) {
                            ...
                            fprintf(fpWrite, "[color=red, label = %d] ", dist[i]);
                        }
                        
                    }
                    else if (marked[j] == 0 && marked[i] != 0) {
                        marked[j] = 1;
                        fprintf(fpWrite, "[color=red, label = %d] ", dist[j]);
                    }
                    else if (marked[i] == 0 && marked[j] != 0) {
                        marked[i] = 1;
                        fprintf(fpWrite, "[color=red, label = %d] ", dist[i]);
                    }
                }
                else if (graph->a[j][i] == -5) {
                    if (j == root) marked[j] = 1;
                ...
                if (j == graph->n -1 && i == graph->n-1 && i != root && j != root && i != dest && j != dest) {
                    fprintf(fpWrite, "\n");
                }
                ...
                if (i == root) {
                    fprintf(fpWrite, "      ");
                    fprintf(fpWrite, "%d  ", i);
                    fprintf(fpWrite, "[color = %s, style = filled] ", "green");
                ...
                if (i == dest) {
                    fprintf(fpWrite, "      ");
                    fprintf(fpWrite, "%d  ", i);
                    fprintf(fpWrite, "[color = %s, style = filled] ", "blue");
...
void dijkstra (GRAPH *graph, int root, int dest) {
    ...
    printShortestPathForDijkstra (graph, root, dest, parent, dist[dest], a_d);
    makeDotFileForDijkstra (graph, root, dest, dist, a_d);
}
